...
gpt_path = "/content/GPT-SoVITS-No-WebUI-Inference/gptsovits_models/pretrained/s1bert25hz-2kh-longer-epoch=68e-step=50232.ckpt"
sovits_path = "/content/GPT-SoVITS-No-WebUI-Inference/gptsovits_models/pretrained/s2G488k.pth"
...
def get_gpt_model(model_name):
    ...
    for r, _, f_list in os.walk(model_dir_path):
        for f in f_list:
            ...
            if f_path.endswith(".ckpt"):
                return f_path
    return gpt_path
                
def get_sovits_model(model_name):
    model_dir_path = os.path.join(models_dir_path, model_name)
    for r, _, f_list in os.walk(model_dir_path):
        for f in f_list:
            f_path = os.path.join(r, f)
            if f_path.endswith(".pth"):
                return f_path
    return sovits_path
...
def gptsovits_inference(model_name, use_pretrained_gpt, use_pretrained_sovits, ref_wav_path, prompt_text, prompt_language, text, text_language, how_to_cut="No slice", top_k=20, top_p=0.6, temperature=0.6, ref_free = False):
    ...
    if model_name:
        if not use_pretrained_gpt:
            gpt_path = get_gpt_model(model_name)
            change_gpt_weights(gpt_path)
        if not use_pretrained_sovits:
            sovits_path = get_sovits_model(model_name)
            change_sovits_weights(sovits_path)
    ...
    while True:
        opt_filename = f"GPTSoVITS_{model_name}_{output_count}.wav"
    ...
    print("Times:\nref_audio: {:.2f}s text: {:.2f}s gpt: {:.2f}s sovits: {:.2f}s".format(*times))
